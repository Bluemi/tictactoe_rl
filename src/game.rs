 ...
 */
fn index_position_to_state_position(index_position: IndexPosition, entry_kind: EntryKind) -> StatePosition {
	let state_position: StatePosition = 1 << entry_kind.get_shift_size();
	return state_position << index_position;
}
...
fn get_possible_state_positions(possible_state_positions: &mut [u32; 9], state: State) -> u32 {
...
pub fn get_random_state_position(state: State, entry_kind: EntryKind, random_generator: &mut RandomGenerator) -> StatePosition {
	...
	return possible_state_positions[index as usize] << entry_kind.get_shift_size();
}
...
pub fn get_interactive_state_position(state: State, entry_kind: EntryKind) -> StatePosition {
	let empty_fields: State = get_empty_fields(state);
	...
	loop {
		...
		if index_position > 8 {
			...
			state_position = index_position_to_state_position(index_position, entry_kind);
		}
...
pub fn toggle_entry_kind(entry_kind: EntryKind) -> EntryKind {
	return match entry_kind {
		EntryKind::X => EntryKind::O,
		EntryKind::O => EntryKind::X,
	}
}
